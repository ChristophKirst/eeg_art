...
>>> viewer = EEGViewer(board=RandomBoard().start())
>>> viewer.show()
>>> viewer.start()
"""
...
class EEGViewer(QtWidgets.QWidget):
    def __init__(
        ...
        self.n_samples = int(self.window_duration * self.sampling_rate)
        self.data_buffer = np.zeros((self.board.n_channels, self.n_samples))
    ...
    def initialize_timeseries_plots(self):
        ...
        for i in range(self.board.n_channels):
            plot = pg.PlotItem()
            curve = pg.PlotCurveItem(y=np.zeros(self.n_samples), pen=(255, 0, 0))
            plot.addItem(curve)
            ...
            view_box.setXRange(0, self.n_samples, padding=0.01)
    ...
    def update(self):
        data = self.board.get_current_data(self.n_samples)
        n_new_points = data.shape[0]
        if n_new_points == 0:
            return

        if n_new_points < self.n_samples:
            self.data_buffer = np.roll(self.data_buffer, -n_new_points, axis=1)
            self.data_buffer[:, -n_new_points:] = data.T
        else:
